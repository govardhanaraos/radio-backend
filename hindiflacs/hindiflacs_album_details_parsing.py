...
router = APIRouter(prefix="/hindiflacs-album-details", tags=["hindiflacs-album-details"])
...
def insert_song(cur, conn, album_id, song):
    cur.execute("""
        INSERT INTO hindiflacs_songs (
            album_id, song_name, song_link, play_link,
            duration, hindiflacs_singers
        )
        VALUES (%s, %s, %s, %s, %s, %s)
        ...
        DO UPDATE SET
            ...
            hindiflacs_singers = EXCLUDED.hindiflacs_singers
        RETURNING id
    """, (
        album_id,
        song["name"],
...
def link_song_singers(cur, conn, song_id, singer_ids):
    for sid in singer_ids:
        cur.execute("""
            INSERT INTO hindiflacs_song_singers (song_id, singer_id)
            VALUES (%s, %s)
            ON CONFLICT DO NOTHING
        """, (song_id, sid))
    conn.commit()
...
def process_one(album_id: int):
    conn, cur = get_connection()
    cur.execute("SELECT album_link FROM hindiflacs_albums_list WHERE id=%s", (album_id,))
    row = cur.fetchone()
    conn.close()
